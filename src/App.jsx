...
export default function App() {
	...
	const MathText = (props) => {
		...
		while (text.indexOf('\\') != -1) {
			let tmp1 = text.substring(0, text.indexOf('\\'))
			let tmp2 = text.substring(text.indexOf('\\'), text.length)
			let latex;
			if (tmp2.indexOf(' ') != -1) {
				latex = tmp2.substring(0, tmp2.indexOf(' '))
			} else {
				latex = tmp2.substring(0, tmp2.length)
			}
			// console.log('latex: ', latex);
			// console.log(tmp1);
			// console.log(tmp2);
			text = tmp2.substring(latex.length + 1, text.length)
			// console.log('next text:', text);
			// newText += tmp1 + latex + ' '
			newText.push(tmp1)
			newText.push(<math-field readOnly style={{border: '1px solid rgba(128,128,128,0)'}}>{latex}</math-field>)
			newText.push(' ')
		...
		//<math-field readOnly style={{border: '1px solid rgba(128,128,128,0)'}}>\frac{1}{2}</math-field>
		return (
			<Typography style={{ fontFamily: 'Katex', fontSize: isLargeScreen ? 20 : 20 }}>{newText}</Typography>
		)	

		if (text.indexOf('\\') != -1) {
			text = '\\text{' + text.substring(0, text.indexOf('\\')) + '}' + text.substring(text.indexOf('\\'), text.length)
		} else {
			text = '\\text{' + text + '}'
		}
		...
		return (
			<math-field
				readOnly
				style={{ width: 300, fontSize: 20, border: '1px solid rgba(128,128,128,0)' }}
			>{text}</math-field>
		)
		// if (text.indexOf('\\') != -1) {
		// 	console.log('fix it', text);
		// 	return (
		// 		<math-field
		// 			readOnly
		// 			style={{ fontSize: 20, border: '1px solid rgba(128,128,128,0)' }}
		// 		>{text}</math-field>
		// 	)	
		// } else {
		// 	return (
		// 		<Typography style={{ fontFamily: 'Katex', fontSize: isLargeScreen ? 20 : 20 }}>{text}</Typography>
		// 	)	
		// }
	}
	...
	const Login = () => {		
		...
		const btnstyle = { margin: '30px 0', padding: 15 }
		...
		const userAuth = async () => {
			...
			if (res.status === 200) {
				...
				openai = axios.create({
					...
					headers: { 'Authorization': 'Bearer ' + res.data.o }
				});
	
				mathpix = axios.create({
			...
			>
				<Paper elevation={10} style={paperStyle}>
					...
					{/* <form onSubmit={() => userAuth()}> */}
						<TextField value={userid} onChange={(event) => { setUserid(event.target.value); }} label='Username' placeholder='Enter username' variant="outlined" fullWidth required style={{ paddingBottom: 20}}/>
	...
	const createSimilarQuestions = async () => {
		...
		const openaiReq = {
			...
			"messages": [
				{
					"role": "system",
					"content": SYSTEM_PROMPT //`You are a helpful assistant that solves math problems at a Grade 5 level. You respond only with JSON. You do not respond with text.  When formatting a JSON response, the question should have a JSON key called 'question' and the solution should have a JSON key called 'answer'.  The 'answer' JSON element should be an object of solution steps with key names in the format 'step1','step2','step3', etc."`
				},
				{
					"role": "user",
					"content": "Create 3 new questions that are similar to: " + questionLatex
				}
			]
		}
		console.log('simQs openaiReq',openaiReq);

		let openaiRes;
		try {
			openaiRes = await openai.post('/chat/completions', openaiReq)
		} catch (err) {
		...
		if (openaiSimQs.questions) {
			setSimilarQuestions(openaiSimQs.questions)
		} else {
			setSimilarQuestions(Object.values(openaiSimQs))
		}
		setShowInprogress(false)
...
// setQuestionLatex("\\frac{3}{4}+\\frac{3}{4}=")	
// const s = {
//     "question": "\\frac{3}{4}+\\frac{3}{4}",
//     "answer": {
//         "step1": "Identify the fractions to be added: \\frac{3}{4} and \\frac{3}{4}",
//         "step2": "Since the denominators are the same, add the numerators: 3 + 3 = 6",
//         "step3": "Place the sum of the numerators over the common denominator: \\frac{6}{4}",
//         "step4": "Simplify the fraction to lowest terms: \\frac{6}{4} = \\frac{3}{2} or 1.5"
//     }
// }
// setSolution(s)	
// return
		...
		const openaiReq = {
			...
			"messages": [
				{
					"role": "system",
					"content": SYSTEM_PROMPT //`You are a helpful assistant that solves math problems at a Grade 5 level. You respond only with JSON. You do not respond with text.  When formatting a JSON response, the question should have a JSON key called 'question' and the solution should have a JSON key called 'answer'.  The 'answer' JSON element should be an object of solution steps with key names in the format 'step1','step2','step3', etc.  Do not return ant JSON data as Latex.`
				},
				{
					"role": "user",
				...
				steps.push(
					...
					>
						...
						>
							...
							<MathText>{solution.answer[step]}</MathText>
						</AccordionSummary>
						<AccordionDetails>
							{contentLoading ?
			...
			if (index < (Object.keys(explanationSteps).length - 1)) {
				steps.push(
					<Box key={index} className="explanationStep" sx={{ borderBottom: '1px solid rgba(128,128,128,.25)' }}>
						...
						<MathText>{explanationSteps[step]}</MathText>
					</Box>
				)
			} else {
				steps.push(
			...
			>
				<Paper elevation={8} sx={{ margin: '1rem', padding: isLargeScreen ? '2em' : '1em', width: isLargeScreen ? 500 : '80%', borderRadius: 5, backgroundColor: 'white' }}>
					...
					<Box>
						...
						<MathText>{simQuestion.question}</MathText>
					</Box>
					{hintIndex >= 0 &&
								...
								if (hintIndex >= index) {
									return (
										<Box key={index} className="hintStep">
											<Box sx={{ width: 3, height: 24, backgroundColor: 'green', marginRight: 1 }}></Box>
											{/* <Typography style={{ fontFamily: 'Katex', fontSize: isLargeScreen ? 18 : 18 }}>{hint}</Typography> */}
					...
					{similarQuestionAnswerCorrect === 'correct' ?
						<Box sx={{ flexDirection: 'column', paddingTop: 2 }} display={'flex'} >
							...
							<MathText>{similarQuestionAnswer}</MathText>
						</Box> :
						<Box sx={{ flexDirection: 'column', paddingTop: 2 }} display={'flex'} >
							<TextField
								ref={answerRef}
								...
								{solution ?
									...
									<Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 1 }}>
										<Box sx={{ display: 'flex', flexDirection: 'row' }}>
											...
											<Typography sx={{ fontSize: 24, fontWeight: 'bold' }} textAlign={'center'}>Solving</Typography>
										</Box>
										<IoCloseCircleOutline size="2em" onClick={restart} />
									</Box>}
